...
public class CrateRegistry {
        ...
        for (File file : files) {
            ...
            if (rewardsSection != null) {
                for (String rewardId : rewardsSection.getKeys(false)) {
                    ...
                    ItemStack displayItem = rSec.getItemStack("display_item");
                    if (displayItem == null) {
                        displayItem = ConfigItemParser.parse(rSec.getConfigurationSection("display_item"));
                    }
                    ...
                    if (winItemsSec != null) {
                        for (String itemKey : winItemsSec.getKeys(false)) {
                            ItemStack winItem = winItemsSec.getItemStack(itemKey);
                            if (winItem == null) {
                                winItem = ConfigItemParser.parse(winItemsSec.getConfigurationSection(itemKey));
                            }
                            if (winItem != null) {
            ...
            // Ambient block effect
            String ambientStr = config.getString("ambient-effect", "NONE");
            crate.setAmbientEffect(AmbientEffect.fromString(ambientStr));
            ...
            int globalIpLimit = plugin.getConfig().getInt("ip-limit.global", -1);
            int crateIpLimit = plugin.getConfig().getInt("ip-limit." + crateId, globalIpLimit);
            crate.setIpLimit(crateIpLimit);
    ...
    public void saveCrate(Crate crate) {
        ...
        plugin.getServer().getScheduler().runTaskAsynchronously(plugin, () -> {
            ...
            config.set("ambient-effect", crate.getAmbientEffect().name());
            
            // IP limit is now managed centrally in config.yml, so we don't save it here.
            config.set("ip-limit", null);
            ...
            for (IReward reward : crate.getRewards()) {
                if (reward instanceof com.pumpkings.pkcrates.core.model.reward.UnifiedReward unReward) {
                    ...
                    if (unReward.getDisplayItem() != null) {
                        if (unReward.getDisplayItem().hasItemMeta() && unReward.getDisplayItem().getItemMeta().hasCustomModelData() == false && unReward.getDisplayItem().getItemMeta().hasEnchants() == false && unReward.getDisplayItem().getItemMeta().hasLore() == false && unReward.getDisplayItem().getItemMeta().hasDisplayName() == false) {
                           ...
                           com.pumpkings.pkcrates.infrastructure.item.ConfigItemSerializer.serialize(unReward.getDisplayItem(), rSec, "display_item");
                        }
                    }
                    ...
                    if (unReward.getItems() != null && !unReward.getItems().isEmpty()) {
                        ...
                        for (org.bukkit.inventory.ItemStack item : unReward.getItems()) {
                            com.pumpkings.pkcrates.infrastructure.item.ConfigItemSerializer.serialize(item, itemsSec, "item_" + i);
                            i++;
                        }
                    }
                }
