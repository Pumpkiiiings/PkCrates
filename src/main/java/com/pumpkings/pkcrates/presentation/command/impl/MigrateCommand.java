...
public class MigrateCommand {
    ...
    public static LiteralCommandNode<CommandSourceStack> build(Plugin plugin,
        ...
        return Commands.literal("migrate")
                ...
                .then(Commands.literal("phoenix")
                        .executes(context -> run(context.getSource().getSender(), plugin, crateRegistry, keyRegistry, databaseManager, true, false))
                        .then(Commands.literal("confirm")
                                .executes(context -> run(context.getSource().getSender(), plugin, crateRegistry, keyRegistry, databaseManager, false, false)))
                        .then(Commands.literal("overwrite")
                                .executes(context -> run(context.getSource().getSender(), plugin, crateRegistry, keyRegistry, databaseManager, false, true))))
                .build();
    }

    private static int run(CommandSender sender, Plugin plugin, CrateRegistry crateRegistry,
                           KeyRegistry keyRegistry, com.pumpkings.pkcrates.infrastructure.database.DatabaseManager databaseManager, boolean dryRun, boolean overwrite) {
        ...
        Bukkit.getScheduler().runTaskAsynchronously(plugin, () -> {
            MigrationReport report = new PhoenixMigrator(plugin).migrate(sourceDirs, dryRun, overwrite, databaseManager);
    ...
    private static void printReport(CommandSender sender, MigrationReport report, boolean dryRun) {
        ...
        sender.sendRichMessage("<gray>Key Files: <white>" + report.getKeyFilesMigrated() + "</white> migrated"
                + (report.getKeyFilesSkipped() > 0 ? " <gray>(skipped <white>" + report.getKeyFilesSkipped() + "</white>)</gray>" : "")
                + (report.getKeyFilesCreated() > 0 ? " <gray>, <white>" + report.getKeyFilesCreated() + "</white> placeholder(s)</gray>" : ""));
        sender.sendRichMessage("<gray>Items: <white>" + report.getItemsFound() + "</white> found, <green>" 
                + report.getItemsSuccessfullyDeserialized() + "</green> deserialized" 
                + (report.getItemsFailed() > 0 ? ", <red>" + report.getItemsFailed() + " failed</red>" : "") + "</gray>");
        sender.sendRichMessage("<gray>Player Keys: <white>" + report.getPlayerKeysFound() + "</white> entries found, <green>" 
                + report.getPlayerKeysMigrated() + "</green> migrated" 
                + (report.getPlayerKeysFailed() > 0 ? ", <red>" + report.getPlayerKeysFailed() + " failed</red>" : "") + "</gray>");
        ...
        printSection(sender, report, MigrationReport.Severity.ERROR, "<red><bold>Errors</bold></red>");
