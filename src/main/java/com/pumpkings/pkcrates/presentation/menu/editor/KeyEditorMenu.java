...
public class KeyEditorMenu extends PkMenu {
    ...
    public void decorate() {
        ...
        Material restMat = config != null ? config.getItemMaterial("items.restriction", Material.REPEATER) : Material.REPEATER;
        ItemStack restrictionIcon = new ItemStack(restMat);
        ItemMeta restMeta = restrictionIcon.getItemMeta();
        if (restMeta != null) {
            Map<String, String> placeholders = new HashMap<>();
            placeholders.put("<restriction_type>", key.getRarityRestriction().name());
            if (config != null) {
                restMeta.displayName(config.getItemName("items.restriction", placeholders));
                restMeta.lore(config.getItemLore("items.restriction", placeholders));
            }
            restrictionIcon.setItemMeta(restMeta);
        }
        setButton(21, new Button(restrictionIcon, event -> {
        ...
        // Rarity Target
        Material targetMat = config != null ? config.getItemMaterial("items.target", Material.PAPER) : Material.PAPER;
        ItemStack targetIcon = new ItemStack(targetMat);
        ...
        if (targetMeta != null) {
            ...
            if (config != null) {
                targetMeta.displayName(config.getItemName("items.target", placeholders));
                targetMeta.lore(config.getItemLore("items.target", placeholders));
            }
            targetIcon.setItemMeta(targetMeta);
        }
        setButton(23, new Button(targetIcon, event -> {
