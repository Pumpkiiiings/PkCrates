    ...
    public void decorate() {
        ...
        if (weightMeta != null) {
            ...
            if (config != null) {
                ...
                weightMeta.displayName(TextUtil.parse("<gold><bold>Probability (Weight)"));
                List<Component> lore = new ArrayList<>();
                lore.add(TextUtil.parse("<gray>Current weight: <white>" + reward.getWeight()));
                lore.add(Component.empty());
                lore.add(TextUtil.parse("<green>▶ Left-Click to add (+1.0)"));
                lore.add(TextUtil.parse("<red>▶ Right-Click to subtract (-1.0)"));
                lore.add(TextUtil.parse("<green>▶ Shift-Left-Click (+10.0)"));
                lore.add(TextUtil.parse("<red>▶ Shift-Right-Click (-10.0)"));
                weightMeta.lore(lore);
            }
        ...
        if (rarityMeta != null) {
            ...
            rarityMeta.displayName(TextUtil.parse("<aqua><bold>Rarity (Category)"));
            List<Component> lore = new ArrayList<>();
            lore.add(TextUtil.parse("<gray>Current rarity: <white>" + currentRarity));
            lore.add(Component.empty());
            lore.add(TextUtil.parse("<green>▶ Left-Click to prompt ID"));
            lore.add(TextUtil.parse("<red>▶ Right-Click to clear"));
            rarityMeta.lore(lore);
        ...
        setButton(11, new Button(rarityIcon, event -> {
            if (event.isRightClick()) {
                ...
                menuManager.getPromptManager().prompt(player, messageManager.getRawMessage(com.pumpkings.pkcrates.infrastructure.config.Messages.PROMPT_RARITY), input -> {
                    if (input.equalsIgnoreCase("cancelar")) {
                        org.bukkit.Bukkit.getScheduler().runTask(plugin, this::open);
                        return;
                    }
                    if (!rarityService.exists(input)) {
                        messageManager.sendMessage(player, com.pumpkings.pkcrates.infrastructure.config.Messages.RARITY_NOT_FOUND);
                    } else {
                        reward.setRarityId(input);
                        crateRegistry.saveCrate(crate);
                        messageManager.sendMessage(player, com.pumpkings.pkcrates.infrastructure.config.Messages.RARITY_ASSIGNED);
                    }
                    org.bukkit.Bukkit.getScheduler().runTask(plugin, this::open);
                });
            }
        ...
        // Broadcast Toggle
        ItemStack broadcastIcon = new ItemStack(Material.OAK_SIGN);
        ItemMeta broadcastMeta = broadcastIcon.getItemMeta();
        if (broadcastMeta != null) {
            broadcastMeta.displayName(TextUtil.parse("<aqua><bold>Global Broadcast"));
            List<Component> lore = new ArrayList<>();
            lore.add(TextUtil.parse("<gray>Should it announce globally?"));
            lore.add(TextUtil.parse("<gray>Status: " + (reward.isBroadcastEnabled() ? "<green>Enabled</green>" : "<red>Disabled</red>")));
            lore.add(Component.empty());
            lore.add(TextUtil.parse("<green>▶ Click to toggle"));
            broadcastMeta.lore(lore);
        ...
        if (cmdMeta != null) {
            ...
            if (config != null) {
                ...
                cmdMeta.displayName(TextUtil.parse("<light_purple><bold>Commands to Execute"));
                List<Component> lore = new ArrayList<>();
                lore.add(TextUtil.parse("<gray>Current commands: <white>" + (reward.getCommands() == null ? 0 : reward.getCommands().size())));
                lore.add(Component.empty());
                lore.add(TextUtil.parse("<green>▶ Click to add a command"));
                lore.add(TextUtil.parse("<red>▶ Shift-Click to clear ALL"));
                cmdMeta.lore(lore);
            }
        ...
        if (displayMeta != null) {
            if (config != null) {
                ...
                displayMeta.displayName(TextUtil.parse("<yellow><bold>Visual Item (Roulette)"));
                List<Component> lore = new ArrayList<>();
                lore.add(TextUtil.parse("<gray>This is the item that will be shown"));
                lore.add(TextUtil.parse("<gray>spinning in the animation."));
                lore.add(Component.empty());
                lore.add(TextUtil.parse("<green>▶ Click with an item from your inventory"));
                lore.add(TextUtil.parse("<green>  to set it as visual."));
        ...
        if (itemsMeta != null) {
            ...
            if (config != null) {
                ...
                itemsMeta.displayName(TextUtil.parse("<aqua><bold>Items to Give"));
                List<Component> lore = new ArrayList<>();
                lore.add(TextUtil.parse("<gray>Amount of items: <white>" + (reward.getItems() == null ? 0 : reward.getItems().size())));
                lore.add(Component.empty());
                lore.add(TextUtil.parse("<green>▶ Click to open container"));
                lore.add(TextUtil.parse("<green>  and add real items."));
                itemsMeta.lore(lore);
            }
