...
public class CrateAnimationMenu extends PaginatedPkMenu<String> {
    ...
    public net.kyori.adventure.text.Component getTitle() {
        ...
        return config != null ? config.getTitle(placeholders) : TextUtil.parse("<light_purple>Animations for: <white>" + crate.getId());
    }
    ...
    public Button createItemButton(String animationId) {
        ...
        if (meta != null) {
            ...
            if (config != null) {
                ...
                meta.displayName(TextUtil.parse("<yellow><bold>" + animationId));
                List<net.kyori.adventure.text.Component> lore = new ArrayList<>();
                lore.add(net.kyori.adventure.text.Component.empty());
                if (isSelected) {
                    lore.add(TextUtil.parse("<green>▶ Selected animation"));
                    meta.addEnchant(org.bukkit.enchantments.Enchantment.UNBREAKING, 1, true);
                    meta.addItemFlags(org.bukkit.inventory.ItemFlag.HIDE_ENCHANTS);
                } else {
                    lore.add(TextUtil.parse("<gray>▶ Click to select"));
                }
                meta.lore(lore);
            }
