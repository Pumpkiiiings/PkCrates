...
public class CrateEditorMenu extends PkMenu {
    ...
    public net.kyori.adventure.text.Component getTitle() {
        ...
        return config != null ? config.getTitle(placeholders) : TextUtil.parse("<dark_gray>Edit Crate <gray>| <white>" + crate.getId());
    }
    ...
    public void decorate() {
        ...
        // Edit Visuals (Prismarine Crystals / slot 29)
        Material visualsMat = config != null ? config.getItemMaterial("items.visuals", Material.PRISMARINE_CRYSTALS) : Material.PRISMARINE_CRYSTALS;
        ItemStack visualsItem = new ItemStack(visualsMat);
        ItemMeta visualsMeta = visualsItem.getItemMeta();
        if (visualsMeta != null) {
            if (config != null) {
                visualsMeta.displayName(config.getItemName("items.visuals", null));
                visualsMeta.lore(config.getItemLore("items.visuals", null));
            } else {
                visualsMeta.displayName(TextUtil.parse("<color:#00CFFF><bold>Visuals"));
            }
            visualsItem.setItemMeta(visualsMeta);
        }
        setButton(29, new Button(visualsItem, event -> {
            new CrateVisualsMenu(plugin, menuManager, player, crate, crateRegistry, this).open();
        }));
