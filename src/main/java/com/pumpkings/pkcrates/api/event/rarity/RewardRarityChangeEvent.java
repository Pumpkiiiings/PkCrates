package com.pumpkings.pkcrates.api.event.rarity;

import com.pumpkings.pkcrates.core.model.reward.IReward;
import org.bukkit.event.Cancellable;
import org.bukkit.event.Event;
import org.bukkit.event.HandlerList;

public class RewardRarityChangeEvent extends Event implements Cancellable {
    private static final HandlerList HANDLERS = new HandlerList();
    
    private final IReward reward;
    private final String oldRarityId;
    private final String newRarityId;
    private boolean cancelled;

    public RewardRarityChangeEvent(IReward reward, String oldRarityId, String newRarityId) {
        this.reward = reward;
        this.oldRarityId = oldRarityId;
        this.newRarityId = newRarityId;
    }

    public IReward getReward() {
        return reward;
    }

    public String getOldRarityId() {
        return oldRarityId;
    }

    public String getNewRarityId() {
        return newRarityId;
    }

    @Override
    public boolean isCancelled() {
        return cancelled;
    }

    @Override
    public void setCancelled(boolean cancel) {
        this.cancelled = cancel;
    }

    @Override
    public HandlerList getHandlers() {
        return HANDLERS;
    }

    public static HandlerList getHandlerList() {
        return HANDLERS;
    }
}
