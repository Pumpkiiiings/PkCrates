...
public class KeyService {
    ...
    private final com.pumpkings.pkcrates.PkCratesPlugin plugin;
    ...
    public KeyService(Plugin plugin, DatabaseManager databaseManager) {
        ...
        this.plugin = (com.pumpkings.pkcrates.PkCratesPlugin) plugin;
    }
    ...
    public void giveKey(Player player, IKey key, int amount) {
        if (key.isVirtual()) {
            databaseManager.addVirtualKeys(player.getUniqueId(), key.getId(), amount).thenRun(() -> {
                plugin.getMessageManager().sendMessage(player, com.pumpkings.pkcrates.infrastructure.config.Messages.KEY_RECEIVED, "<amount>", String.valueOf(amount), "<key>", key.getId());
            });
            return;
        }
